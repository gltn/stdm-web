...
from .renameable_dict import RenameableKeyDict
...
class EntitySupportingDocument(Entity):
    ...
    def __init__(self, profile, parent_entity):
        ...
        if self._doc_types_value_list is None:
            ...
            # Add a default type
            self._doc_types_value_list.add_value('General')
    ...
    def _update_fk_references(self):
        ...
        if entity_id is None:
            err = '{0} does not have an id column. This is required in order ' \
                  'to link it to the supporting document table through this ' \
                  'association table.'.format(self.parent_entity.name)
